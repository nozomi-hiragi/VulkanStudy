...
class DescriptorPoolObject : public AbstractFactory<DescriptorSetObject, DeviceObject, const std::vector<VkDescriptorSetLayout>&> {
public:
...
private:
  ...
  std::shared_ptr<DescriptorSetObject> _createCore(const std::vector<VkDescriptorSetLayout>& descriptor_set_layouts) {
    ...
    descriptor_set_info.descriptorSetCount = static_cast<uint32_t>(descriptor_set_layouts.size());
    descriptor_set_info.pSetLayouts = descriptor_set_layouts.data();
